...
const AudioManager = {
    ...
    // Dans la méthode playMusicFromBuffer, remplacez la partie existante par ceci
playMusicFromBuffer: function() {
    ...
    // Créer une nouvelle source à partir du buffer
    this.musicSource = this.context.createBufferSource();
    this.musicSource.buffer = this.musicBuffer;
    
    // MODIFICATION : Utiliser une technique avancée de bouclage
    // Au lieu de simplement activer la boucle, nous allons spécifier les points exacts
    const buffer = this.musicBuffer;
    const duration = buffer.duration;
    
    // Ajuster les points de boucle avec une petite marge pour éviter les clics
    const loopStartInSeconds = 0;
    const loopEndInSeconds = duration;
    
    // Définir les points de boucle précis
    this.musicSource.loop = true;
    this.musicSource.loopStart = loopStartInSeconds;
    this.musicSource.loopEnd = loopEndInSeconds;
    ...
    // Démarrer la lecture en spécifiant le moment exact
    this.musicSource.start(0, 0, duration);
    
    console.log(`Boucle musicale configurée: durée=${duration}s, début=${loopStartInSeconds}s, fin=${loopEndInSeconds}s`);
},
...
playMusicFallback: function(path) {
   ...
   pauseAll: function() {
    ...
    if (this.isInitialized && this.context.state === 'suspended') {
        ...
        if (this.musicBuffer && this.currentMusicPath && !this.musicSource) {
            this.playMusicFromBuffer();
        }
    }
