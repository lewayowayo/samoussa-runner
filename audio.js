...
const AudioManager = {
    ...
    // Remplacez la méthode playMusicFromBuffer par cette version améliorée
playMusicFromBuffer: function() {
    ...
    // Ajouter un gestionnaire pour redémarrer la musique si elle s'arrête
    this.musicSource.onended = () => {
        console.log("Musique arrêtée de manière inattendue, tentative de redémarrage");
        // Si la musique s'arrête pour une raison quelconque et que nous ne sommes pas en mode muet
        if (!this.isMuted) {
            // Petit délai pour éviter des redémarrages en cascade
            setTimeout(() => {
                this.playMusicFromBuffer();
            }, 50);
        }
    ...
    // Démarrer la lecture sans paramètres supplémentaires
